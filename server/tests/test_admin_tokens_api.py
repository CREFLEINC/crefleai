...
def test_토큰_생성과_목록(admin_client):
    res = admin_client.post(
        "/api/admin/tokens", json={"user_name": "홍길동", "purpose": "프로토타입"}
    )
    ...
    listed = admin_client.get("/api/admin/tokens").json()["tokens"]
    assert len(listed) == 1
...
def test_폐기하면_검증_실패(admin_client):
    ...
    res = admin_client.delete(f"/api/admin/tokens/{created['jti']}")
    assert res.status_code == 200
...
def test_없는_토큰_폐기는_404(admin_client):
    assert admin_client.delete("/api/admin/tokens/없는-jti").status_code == 404


def test_로그인_없이_토큰_API_접근은_401(client):
    assert client.get("/api/admin/tokens").status_code == 401
    assert client.post("/api/admin/tokens", json={"user_name": "a", "purpose": "b"}).status_code == 401
